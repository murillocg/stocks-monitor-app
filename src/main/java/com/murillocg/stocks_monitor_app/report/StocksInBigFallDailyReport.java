...
public class StocksInBigFallDailyReport implements ApplicationListener<StockPricesUpdated> {
    ...
    public StocksInBigFallDailyReport(WalletStocksRepository walletStocksRepository, StockKeyPricesRepository stockKeyPricesRepository, EarningsCalculator earningsCalculator) {
    ...
    public void onApplicationEvent(StockPricesUpdated event) {
        ...
        for (String stock : myStocks) {
            var prices = stockKeyPricesRepository.findById(stock)
                    .orElseThrow(() -> new RuntimeException("Stock not found in the stockKeyPriceRepository: " + stock));
    ...
    record ReportRecord(String symbol, double highPrice, LocalDate highPriceDate, double currentPrice, double earnings) {}
